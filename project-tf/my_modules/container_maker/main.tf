/* Understanding Modules
   my_modules/container_maker/main.tf - the child module contains the work we want to actually do.
   Think of it like you might a function in other coding languages. In Ansible, it is not too unlike a "role" */
...
terraform {
  required_providers {   # without the required providers inside of the module, the root module may have problems when calling the module
    docker = {
      ...
      # version = "2.22.1"   # Do not use ~> (or other maximum-version constraints) for modules you intend to reuse across many configurations... 
      # Specify a minimum version.. let the root module manage the maximum version.
    }
  }
}

resource "docker_image" "nginx" {
  name         = "nginx:1.23.4"      # within the child module we can make a decision regarding values we have mapped.
                                     # Perhaps we want to statically map the image name and tag, as opposed to allowing the user to map it with a variable
